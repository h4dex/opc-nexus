...
const NEXUS_THEME_EVENT = "opc-nexus-theme-change";
const NEXUS_TYPOGRAPHY: ThemeTypography = {
  fontSans: 'Inter, "Microsoft YaHei UI", "Segoe UI", sans-serif',
  fontMono: '"Cascadia Mono", Consolas, monospace',
  fontDisplay: 'Inter, "Microsoft YaHei UI", "Segoe UI", sans-serif',
  baseSize: "15px",
  lineHeight: "1.55",
  letterSpacing: "0",
};
const NEXUS_LAYOUT: ThemeLayout = { radius: "0.5rem", density: "comfortable" };
const NEXUS_THEMES: Record<"dark" | "light", DashboardTheme> = {
  dark: {
    name: "opc-nexus-dark",
    label: "OPC-Nexus Dark",
    description: "Host-controlled OPC-Nexus dark theme",
    palette: {
      background: { hex: "#0b0e14", alpha: 1 },
      midground: { hex: "#eef2fc", alpha: 1 },
      foreground: { hex: "#4d6bfe", alpha: 1 },
      warmGlow: "rgba(77, 107, 254, 0.12)",
      noiseOpacity: 0,
    },
    typography: NEXUS_TYPOGRAPHY,
    layout: NEXUS_LAYOUT,
    colorOverrides: {
      card: "#111722",
      cardForeground: "#eef2fc",
      popover: "#111722",
      popoverForeground: "#eef2fc",
      primary: "#4d6bfe",
      primaryForeground: "#ffffff",
      secondary: "#151c29",
      secondaryForeground: "#eef2fc",
      muted: "#1c2433",
      mutedForeground: "#aab4c8",
      accent: "#202a3d",
      accentForeground: "#eef2fc",
      border: "#2c3443",
      input: "#343e50",
      ring: "#4d6bfe",
    },
    terminalBackground: "#0b0e14",
    terminalForeground: "#eef2fc",
  },
  light: {
    name: "opc-nexus-light",
    label: "OPC-Nexus Light",
    description: "Host-controlled OPC-Nexus light theme",
    palette: {
      background: { hex: "#f7f8fa", alpha: 1 },
      midground: { hex: "#182033", alpha: 1 },
      foreground: { hex: "#4d6bfe", alpha: 1 },
      warmGlow: "rgba(77, 107, 254, 0.08)",
      noiseOpacity: 0,
    },
    typography: NEXUS_TYPOGRAPHY,
    layout: NEXUS_LAYOUT,
    colorOverrides: {
      card: "#ffffff",
      cardForeground: "#182033",
      popover: "#ffffff",
      popoverForeground: "#182033",
      primary: "#4d6bfe",
      primaryForeground: "#ffffff",
      secondary: "#eef1f6",
      secondaryForeground: "#182033",
      muted: "#e8ecf3",
      mutedForeground: "#657086",
      accent: "#e3e9ff",
      accentForeground: "#182033",
      border: "#d5dbe6",
      input: "#cbd3e1",
      ring: "#4d6bfe",
    },
    terminalBackground: "#f7f8fa",
    terminalForeground: "#182033",
  },
};
...
export function ThemeProvider({ children }: { children: ReactNode }) {
  const nexusProjectMode = typeof window !== "undefined" && Boolean(window.__OPC_NEXUS_PROJECT_MODE__);
  const [nexusTheme, setNexusTheme] = useState<"dark" | "light">(() => {
    if (typeof window === "undefined") return "dark";
    if (window.__OPC_NEXUS_THEME__) return window.__OPC_NEXUS_THEME__;
  ...
  const resolveTheme = useCallback(
    (name: string): DashboardTheme => {
      if (nexusProjectMode) return NEXUS_THEMES[nexusTheme];
      return (
        BUILTIN_THEMES[name] ??
        userThemeDefs[name] ??
        defaultTheme
    ...
    [nexusProjectMode, nexusTheme, userThemeDefs],
  );
  ...
  useEffect(() => {
    if (!nexusProjectMode || typeof window === "undefined") return;
    const applyHostTheme = (detail: unknown) => {
      if (detail !== "dark" && detail !== "light") return;
      window.__OPC_NEXUS_THEME__ = detail;
      document.documentElement.dataset.theme = detail;
      document.documentElement.classList.toggle("dark", detail === "dark");
      setNexusTheme(detail);
    };
    const receiveTheme = (event: Event) => applyHostTheme((event as CustomEvent<unknown>).detail);
    window.addEventListener(NEXUS_THEME_EVENT, receiveTheme);
    // Main may apply the theme just before React effects subscribe. Reading
    // the host-owned value here closes that startup race.
    applyHostTheme(window.__OPC_NEXUS_THEME__);
    return () => window.removeEventListener(NEXUS_THEME_EVENT, receiveTheme);
  }, [nexusProjectMode]);

  // Load server-side themes (built-ins + user YAMLs) once on mount.
  useEffect(() => {
    if (nexusProjectMode) return;
    let cancelled = false;
    api
      .getThemes()
      .then((resp) => {
  ...
  const value = useMemo<ThemeContextValue>(
    () => ({
      ...
      themeName: nexusProjectMode ? NEXUS_THEMES[nexusTheme].name : themeName,
      availableThemes,
      setTheme,
      fontId,
    ...
    [themeName, availableThemes, setTheme, resolveTheme, fontId, setFont, nexusProjectMode, nexusTheme],
  );
