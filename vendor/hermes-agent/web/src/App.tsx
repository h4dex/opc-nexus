...
import { NexusProjectBar } from "@/components/NexusProjectBar";
...
function RootRedirect() {
  return <Navigate to={window.__OPC_NEXUS_PROJECT_MODE__ ? "/chat" : "/sessions"} replace />;
}
...
export default function App() {
  ...
  const nexusProjectMode = window.__OPC_NEXUS_PROJECT_MODE__;
  const embeddedNexusDesktop = nexusProjectMode === "desktop";
  const mobileNexusOperator = nexusProjectMode === "mobile-operator";
  const showHermesChrome = !embeddedNexusDesktop && !mobileNexusOperator;
  const { manifests, loading: pluginsLoading } = usePlugins(!nexusProjectMode);
  ...
  const builtinNav = useMemo(() => {
    ...
    return nexusProjectMode ? visible.filter((item) => item.path === "/chat") : visible;
  }, [embeddedChat, nexusProjectMode, showTokenAnalytics]);

  const sidebarNav = useMemo(
    () => partitionSidebarNav(builtinNav, nexusProjectMode ? [] : manifests),
    [builtinNav, manifests, nexusProjectMode],
  );
  ...
  return (
    ...
    >
      ...
      {!nexusProjectMode && <ProfileScopeBanner />}
      {nexusProjectMode && !embeddedNexusDesktop && <NexusProjectBar />}

      <div className={cn(
        "flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden",
        !embeddedNexusDesktop && !nexusProjectMode && "pt-14 lg:pt-0",
      )}>
        <div className="flex min-h-0 min-w-0 flex-1">
          {showHermesChrome && <aside
            id="app-sidebar"
          ...
          >
            ...
            {!nexusProjectMode && <ProfileSwitcher collapsed={isDesktopCollapsed} />}
          ...
          <PageHeaderProvider pluginTabs={pluginTabMeta}>
            <div
              className={cn(
                ...
                embeddedNexusDesktop || mobileNexusOperator ? "p-0" : "px-3 sm:px-6",
                showHermesChrome && (isChatRoute
                ...
                {embeddedChat &&
                  ...
                  (pluginsLoading ? (
                    ...
                    >
                      ...
                      >
                        {nexusProjectMode
                          ? <NexusChatPage isActive={isChatRoute} />
