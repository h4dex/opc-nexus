...
export function ProfileProvider({ children }: { children: ReactNode }) {
  ...
  const nexusProjectMode = Boolean(window.__OPC_NEXUS_PROJECT_MODE__);
  ...
  const [profile, setProfileState] = useState(
    () => nexusProjectMode ? "" : searchParams.get("profile") ?? "",
  );
  ...
  const urlProfile = nexusProjectMode ? null : searchParams.get("profile");
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [nexusProjectMode, pathname, profile]);
  ...
  const setProfile = useCallback(
    ...
    [nexusProjectMode, setSearchParams],
  );
