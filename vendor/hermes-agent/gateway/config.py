...
def load_gateway_config() -> GatewayConfig:
    ...
    # OPC-Nexus owns all external messaging channels. A project-scoped Hermes
    # service may expose only its loopback API server, even when the host
    # process inherited unrelated channel credentials from the machine.
    if os.getenv("HERMES_NEXUS_PROJECT_ID", "").strip():
        ...
        config.platforms = (
            ...
            if api_server is not None and api_server.enabled
            else {}
        )
