...
try:
    ...
    ClientConnectionError = ConnectionError  # type: ignore[assignment,misc]
...
def _get_api_server_toolsets(config: dict, *, include_default_mcp_servers: bool = True) -> set[str]:
    """Resolve the API surface, including the mandatory Nexus host tools in project mode."""
    from hermes_cli.tools_config import _get_platform_tools

    nexus_project = bool(os.getenv("HERMES_NEXUS_PROJECT_ID", "").strip())
    if nexus_project:
        # Import before resolving the registry-backed toolset. Project mode is
        # a host contract, so a dashboard tool toggle must not remove it.
        from tools import nexus_tool as _nexus_tool  # noqa: F401
    ...
    if nexus_project:
        ...
        # a question instead of persisting it through OPC-Nexus.
        enabled.add("clarify")
        enabled.add("planning")
    return enabled
...
_NEXUS_MEMORY_MODES = frozenset({"long_term", "short_term", "none"})
_NEXUS_MEMORY_SCOPE_RE = re.compile(r"^employee-[0-9a-f]{32}$")


def _parse_nexus_memory_policy(
    body: Dict[str, Any],
    *,
    stored: Optional[Dict[str, Any]] = None,
    required: bool = False,
) -> Optional[Dict[str, str]]:
    """Validate the Main-owned per-conversation memory contract."""
    has_mode = "nexus_memory_mode" in body
    has_scope = "nexus_memory_scope" in body
    if has_mode != has_scope:
        raise ValueError("nexus_memory_mode and nexus_memory_scope must be supplied together")

    def _normalize(candidate: Any) -> Optional[Dict[str, str]]:
        if candidate is None:
            return None
        if not isinstance(candidate, dict):
            raise ValueError("stored Nexus memory policy is invalid")
        mode = candidate.get("mode")
        scope = candidate.get("scope")
        if mode not in _NEXUS_MEMORY_MODES:
            raise ValueError("nexus_memory_mode must be long_term, short_term, or none")
        if scope != "project" and not (
            isinstance(scope, str) and _NEXUS_MEMORY_SCOPE_RE.fullmatch(scope)
        ):
            raise ValueError("nexus_memory_scope is invalid")
        return {"mode": mode, "scope": scope}

    persisted = _normalize(stored)
    requested = _normalize({
        "mode": body.get("nexus_memory_mode"),
        "scope": body.get("nexus_memory_scope"),
    }) if has_mode else None
    if requested is not None and persisted is not None and requested != persisted:
        raise ValueError("Nexus memory policy cannot change within an existing session")
    policy = requested or persisted
    if required and policy is None:
        raise ValueError("Nexus sessions require an explicit memory policy")
    return policy


def _nexus_memory_directory(policy: Dict[str, str]) -> str:
    """Resolve a validated policy to a directory contained by HERMES_HOME."""
    from hermes_constants import get_hermes_home

    home = get_hermes_home().resolve()
    scope = policy["scope"]
    target = home if scope == "project" else home / "memories" / "employees" / scope
    target.mkdir(parents=True, exist_ok=True)
    resolved = target.resolve()
    try:
        resolved.relative_to(home)
    except ValueError as exc:
        raise ValueError("Nexus memory directory escaped HERMES_HOME") from exc
    return str(resolved)
...
class APIServerAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Nexus session-chat streams need an acknowledged stop boundary. A
        # disconnected SSE socket alone cannot prove that the executor thread
        # and transcript finalizer have stopped before Main starts the next
        # queued turn for the same conversation.
        self._active_session_runs: Dict[str, Dict[str, Any]] = {}
    ...
    def _http_route_table(self) -> List[tuple]:
        ...
        routes: List[tuple] = [
            ...
            ("POST", "/api/sessions/{session_id}/interrupt", self._handle_session_interrupt),
    ...
    def _persist_nexus_memory_policy(
        self,
        session_id: str,
        policy: Dict[str, str],
        *,
        ...
        """Merge the Nexus memory contract without clobbering runtime locks."""
        db = self._ensure_session_db()
        if db is None:
            return False
        ...
        def _do(conn):
            ...
            config = self._parse_session_model_config(raw)
            config["nexus_memory_policy"] = policy
            if managed:
                config["nexus_managed"] = True
            conn.execute(
        ...
        try:
            ...
            logger.warning(
                "[%s] failed to persist Nexus memory policy for %s",
                self.name,
                session_id,
                exc_info=True,
            )
    ...
    def _create_agent(
        ...
        enabled_toolsets = sorted(_get_api_server_toolsets(user_config))
        ...
        if memory_policy is not None:
            long_term_memory = memory_policy["mode"] == "long_term"
            # Nexus owns the namespace. External memory providers stay off
            # because their scope cannot be proven to match an employee scope.
            agent_kwargs.update({
                "skip_memory": True,
                "memory_enabled": long_term_memory,
                "user_profile_enabled": long_term_memory,
                "memory_dir": (
                    _nexus_memory_directory(memory_policy)
                    if long_term_memory
                    else None
                ),
                "disabled_toolsets": [] if long_term_memory else ["memory"],
            })
        if os.getenv("HERMES_NEXUS_PROJECT_ID", "").strip():
            from tools.nexus_tool import nexus_clarify_callback
            agent_kwargs["clarify_callback"] = nexus_clarify_callback(session_id or "")
    ...
    async def _handle_capabilities(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({
            ...
            "endpoints": {
                ...
                "session_interrupt": {"method": "POST", "path": "/api/sessions/{session_id}/interrupt"},
    ...
    async def _handle_toolsets(self, request: "web.Request") -> "web.Response":
        ...
        try:
            ...
            config = load_config()
            enabled_toolsets = _get_api_server_toolsets(
                config,
            ...
            if "planning" in enabled_toolsets and not any(item["name"] == "planning" for item in data):
                data.append({
                    "name": "planning",
                    "label": "OPC-Nexus Governance",
                    "description": "Project-scoped planning, employee dispatch, and MCP bridge",
                    "enabled": True,
                    "configured": True,
                    "tools": sorted(set(resolve_toolset("planning"))),
                })
        except Exception:
    ...
    async def _handle_create_session(self, request: "web.Request") -> "web.Response":
        ...
        requested_source = str(body.get("source") or "api_server").strip()
        source = self._normalize_session_source(requested_source)
        nexus_managed = (
            requested_source == "nexus"
            or bool(os.getenv("HERMES_NEXUS_PROJECT_ID", "").strip())
        )
        try:
            nexus_memory_policy = _parse_nexus_memory_policy(
                body,
                required=nexus_managed,
            )
        except ValueError as exc:
            return web.json_response(
                _openai_error(str(exc), code="invalid_nexus_memory_policy"),
                status=400,
            )
        runtime_request = self._session_runtime_request_from_body(body)
        ...
        if nexus_memory_policy is not None:
            model_config = model_config or {}
            model_config["nexus_memory_policy"] = nexus_memory_policy
            if nexus_managed:
                model_config["nexus_managed"] = True
    ...
    async def _handle_fork_session(self, request: "web.Request") -> "web.Response":
        ...
        source_nexus_managed = source_config.get("nexus_managed") is True
        try:
            source_memory_policy = _parse_nexus_memory_policy(
                {},
                stored=source_config.get("nexus_memory_policy"),
                required=source_nexus_managed,
            )
        except ValueError as exc:
            return web.json_response(
                _openai_error(str(exc), code="invalid_nexus_memory_policy"),
        ...
        try:
            ...
            return web.json_response(_openai_error(str(exc), code="invalid_title"), status=400)
        if source_memory_policy is not None:
            persisted = await asyncio.to_thread(
                self._persist_nexus_memory_policy,
                fork_id,
                source_memory_policy,
                managed=source_nexus_managed,
            )
            if not persisted:
                await asyncio.to_thread(db.delete_session, fork_id)
                return web.json_response(
                    _openai_error(
                        "Could not inherit the Nexus memory policy",
                        code="nexus_memory_policy_persistence_failed",
                    ),
                    status=500,
                )
    ...
    async def _handle_session_chat(self, request: "web.Request") -> "web.Response":
        ...
        session_config = self._parse_session_model_config(session.get("model_config"))
        nexus_managed = bool(
            session_config.get("nexus_managed") is True
            or session.get("source") == "nexus"
            or os.getenv("HERMES_NEXUS_PROJECT_ID", "").strip()
        )
        try:
            memory_policy = _parse_nexus_memory_policy(
                body,
                stored=session_config.get("nexus_memory_policy"),
                required=nexus_managed,
            )
        except ValueError as exc:
            return web.json_response(
        ...
        stored_memory_policy = session_config.get("nexus_memory_policy")
        if memory_policy is not None and (
            stored_memory_policy is None
            or (nexus_managed and session_config.get("nexus_managed") is not True)
        ):
            persisted = await asyncio.to_thread(
                self._persist_nexus_memory_policy,
                session_id,
                memory_policy,
                managed=nexus_managed,
            )
            if not persisted and stored_memory_policy is None:
                return web.json_response(
                    _openai_error(
                        "Could not persist the Nexus memory policy",
                        code="nexus_memory_policy_persistence_failed",
                    ),
                    status=500,
                )
        ...
        history = (
            []
            if memory_policy is not None and memory_policy["mode"] == "none"
            else await self._conversation_history_for_session(session_id)
        )
    ...
    async def _interrupt_nexus_session_run(
        self,
        state: Dict[str, Any],
        reason: str,
        *,
        ...
        """Interrupt one Nexus session turn and wait for transcript finalization."""
        ...
        while agent is None and time.monotonic() < deadline:
            if isinstance(done, asyncio.Event) and done.is_set():
                return True
            await asyncio.sleep(0.02)
            agent = agent_ref[0] if isinstance(agent_ref, list) and agent_ref else None
        if agent is not None and not state.get("interrupt_sent"):
            state["interrupt_sent"] = True
            try:
                agent.interrupt(reason)
                logger.info("[api_server] interrupted Nexus session agent after cancellation request")
            except Exception:
                state["interrupt_sent"] = False
                logger.debug("[api_server] failed to interrupt Nexus session", exc_info=True)
        if not isinstance(done, asyncio.Event):
            return False
        try:
            ...
            logger.warning(
                "[api_server] Nexus session run %s did not settle within %.1fs",
                state.get("run_id"),
                wait_seconds,
            )
            return False
    ...
    async def _handle_session_interrupt(self, request: "web.Request") -> "web.Response":
        """POST /api/sessions/{session_id}/interrupt — acknowledged Nexus stop."""
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        session_id = request.match_info["session_id"]
        ...
        if not re.fullmatch(r"run_[0-9a-f]{32}", run_id):
            return web.json_response(
                _openai_error("run_id is invalid", code="invalid_run_id"),
                status=400,
            )
        state = self._active_session_runs.get(run_id)
        if not state or state.get("session_id") != session_id:
            return web.json_response(
                _openai_error("The session turn is no longer active", code="session_turn_not_active"),
                status=409,
            )
        reason = str(body.get("reason") or "Cancelled by the Nexus project owner")[:500]
        settled = await self._interrupt_nexus_session_run(state, reason)
        return web.json_response(
            {
                "object": "hermes.session.interrupt",
                "session_id": session_id,
                "run_id": run_id,
                "status": "cancelled" if settled else "stopping",
                "settled": settled,
            },
            status=200 if settled else 202,
        )

    async def _close_nexus_transcript(
        self,
        session_id: str,
        user_message: Any,
        closure: str,
    ) -> None:
        """Persist a terminal assistant fact for a Nexus turn."""
        db = await self._ensure_session_db_async()
        resolved_id = await asyncio.to_thread(db.resolve_resume_session_id, session_id)
        ...
        if not messages:
            ...
            await asyncio.to_thread(db.append_message, resolved_id, "user", content)
            messages = [{"role": "user", "content": content}]
        tail = messages[-1] if isinstance(messages[-1], dict) else {}
        if tail.get("role") != "assistant":
            await asyncio.to_thread(db.append_message, resolved_id, "assistant", closure)
    ...
    async def _handle_session_chat_stream(self, request: "web.Request") -> "web.StreamResponse":
        ...
        self._active_session_runs[run_id] = run_state
        ...
        async def _run_and_signal() -> None:
            try:
                ...
                if interrupted:
                    await self._close_nexus_transcript(
                        effective_session_id,
                        user_message,
                        "Task cancelled by the user. Do not continue this instruction unless "
                        "the user explicitly asks to run it again.",
                    )
                elif isinstance(result, dict) and result.get("failed"):
                    failure = _redact_api_error_text(final_response, limit=800).strip()
                    await self._close_nexus_transcript(
                        effective_session_id,
                        user_message,
                        "Task failed before completion"
                        + (f": {failure}" if failure else ".")
                        + " Do not treat this instruction as completed or continue it "
                        "unless the user explicitly retries it.",
                    )
                effective_runtime = {}
                if isinstance(result, dict):
                ...
                loop.call_later(60.0, self._active_session_runs.pop, run_id, None)
        ...
        try:
            ...
            await self._interrupt_nexus_session_run(
                run_state,
                "Nexus session SSE client disconnected",
            )
            raise
            ...
            logger.debug("[api_server] Nexus session SSE client disconnected: %s", exc)
        except Exception as exc:
            await self._interrupt_nexus_session_run(
                run_state,
                "Nexus session SSE client disconnected",
                    ...
                    if include_runtime:
                        ...
                        if confirmed_runtime_lock:
                            ...
                            provider_matches = (
                                ...
                                # Named custom providers are routable identities,
                                # while AIAgent exposes their shared runtime class.
                                or (
                                    expected_provider.startswith("custom:")
