...
def init_agent(
    ...
    _memory_override_requested = (
        memory_dir is not None
        or memory_enabled is not None
        or user_profile_enabled is not None
    )
    if not skip_memory or _memory_toolset_requested or _memory_override_requested:
        try:
            mem_config = _agent_cfg.get("memory", {})
            agent._memory_enabled = (
                bool(memory_enabled)
                if memory_enabled is not None
                else mem_config.get("memory_enabled", False)
            )
            agent._user_profile_enabled = (
                bool(user_profile_enabled)
