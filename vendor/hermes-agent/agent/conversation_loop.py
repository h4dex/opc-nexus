...
def _flatten_completed_tool_history_for_compat(
    messages: List[Dict[str, Any]],
) -> bool:
    """Downgrade completed OpenAI tool exchanges to ordinary text in-place.

    Some OpenAI-compatible relays accept a valid ``tool_calls`` response but
    reject the required follow-up containing the assistant call plus
    ``role=tool`` result with a generic HTTP 400. The tool already ran at that
    point. An API-only textual receipt lets the model finish without repeating
    the side effect; callers must not pass the canonical session list here.
    """
    ...
    for message in messages:
        ...
        if role == "assistant" and isinstance(tool_calls, list) and tool_calls:
            ...
            visible_text = visible.strip() if isinstance(visible, str) else ""
            summary = (
                f"[Completed tool call: {', '.join(names)}]"
                if names else "[Completed tool call]"
            )
            flattened.append({
                "role": "assistant",
        ...
        if role == "tool":
            ...
            block = f"[Result from completed tool {tool_name}]\n{rendered}"
            if flattened and flattened[-1].get("role") == "user" and flattened[-1].get("_tool_compat"):
                flattened[-1]["content"] = f"{flattened[-1]['content']}\n\n{block}"
            else:
                flattened.append({"role": "user", "content": block, "_tool_compat": True})
            changed = True
            continue
        flattened.append(message)
    ...
    while (api_call_count < agent.max_iterations and agent.iteration_budget.remaining > 0) or agent._budget_grace_call:
        ...
        while retry_count < max_retries:
            ...
            try:
                ...
                # A custom relay can accept the first tool request but route
                # the follow-up to an upstream that rejects role=tool history
                # with only a generic 400. Preserve the real result as text in
                # the API-only copy and retry once. The canonical transcript
                # and completed side effect remain untouched.
                if (
                    status_code == 400
                    and agent.api_mode == "chat_completions"
                    and getattr(agent, "provider", "") == "custom"
                    and classified.reason in {FailoverReason.format_error, FailoverReason.unknown}
                    and not _retry.tool_history_compat_retry_attempted
                    and any(
                        isinstance(_message, dict) and _message.get("role") == "tool"
                        for _message in api_messages
                    )
                ):
                    _retry.tool_history_compat_retry_attempted = True
                    if _flatten_completed_tool_history_for_compat(api_messages):
                        agent._buffer_vprint(
                            "⚠️  Provider rejected completed tool history — "
                            "retrying once with a text-compatible tool receipt."
                        )
                        logger.warning(
                            "%sOpenAI-compatible tool-history recovery: "
                            "flattened API-only transcript for one retry",
                            agent.log_prefix,
                        )
                        continue
