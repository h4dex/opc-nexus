...
def check_delegate_requirements() -> bool:
    """Hide native fan-out when OPC-Nexus owns delegation governance."""
    return not bool(os.getenv("HERMES_NEXUS_HOST_URL", "").strip())
...
def delegate_task(
    ...
    if os.getenv("HERMES_NEXUS_HOST_URL", "").strip():
        return tool_error(
            "Native delegate_task is disabled in an OPC-Nexus project. "
            "Use nexus_delegate_task for one employee or clarify and nexus_submit_plan for a governed team plan."
        )
