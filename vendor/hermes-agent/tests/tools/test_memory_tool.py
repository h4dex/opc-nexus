...
class TestMemoryStorePersistence:
    ...
    def test_custom_directories_are_isolated(self, tmp_path):
        employee_a = MemoryStore(memory_dir=tmp_path / "employee-a")
        employee_b = MemoryStore(memory_dir=tmp_path / "employee-b")
        employee_a.load_from_disk()
        employee_b.load_from_disk()

        employee_a.add("memory", "employee A private fact")
        employee_b.add("memory", "employee B private fact")

        reloaded_a = MemoryStore(memory_dir=tmp_path / "employee-a")
        reloaded_b = MemoryStore(memory_dir=tmp_path / "employee-b")
        reloaded_a.load_from_disk()
        reloaded_b.load_from_disk()
        assert reloaded_a.memory_entries == ["employee A private fact"]
        assert reloaded_b.memory_entries == ["employee B private fact"]
