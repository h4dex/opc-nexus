...
def _create_session_app(adapter: APIServerAdapter) -> web.Application:
    ...
    app.router.add_post("/api/sessions/{session_id}/interrupt", adapter._handle_session_interrupt)
    return app
...
async def test_capabilities_advertises_session_control_surface(adapter):
    ...
    assert features["session_interrupt"] is True
    ...
    assert data["endpoints"]["session_interrupt"] == {
        "method": "POST",
...
@pytest.mark.asyncio
async def test_session_chat_stream_disconnect_interrupts_real_agent(adapter, session_db):
    session_id = session_db.create_session("stream-cancel-session", "api_server")
    agent = Mock()

    async def fake_run(**kwargs):
        # Match _run_agent's executor behavior: cancelling the awaitable does
        # not stop the already-started thread that constructs and runs AIAgent.
        def run_in_executor():
            time.sleep(0.05)
            kwargs["agent_ref"][0] = agent
            while not agent.interrupt.called:
                time.sleep(0.01)
            return {
        ...
        return await asyncio.to_thread(run_in_executor)
    ...
    agent.interrupt.assert_called_once_with("Nexus session SSE client disconnected")

    messages = session_db.get_messages_as_conversation(session_id)
    assert [(item["role"], item["content"]) for item in messages[-2:]] == [
        ("user", "long task"),
        (
            "assistant",
...
@pytest.mark.asyncio
async def test_session_interrupt_endpoint_waits_for_executor_and_closes_transcript(adapter, session_db):
    session_id = session_db.create_session("stream-explicit-cancel", "api_server")
    agent = Mock()

    async def fake_run(**kwargs):
        def run_in_executor():
            session_db.append_message(session_id, "user", "never continue this")
            kwargs["agent_ref"][0] = agent
            while not agent.interrupt.called:
                time.sleep(0.01)
            return {
    ...
    with patch.object(adapter, "_run_agent", side_effect=fake_run):
        async with TestClient(TestServer(app)) as cli:
            stream = await cli.post(
                f"/api/sessions/{session_id}/chat/stream",
                json={"message": "never continue this"},
            )
            assert stream.status == 200
            run_id = stream.headers["X-Hermes-Run-Id"]
            interrupt = await cli.post(
                f"/api/sessions/{session_id}/interrupt",
                json={"run_id": run_id, "reason": "owner cancelled"},
            )
            assert interrupt.status == 200, await interrupt.text()
            payload = await interrupt.json()
            assert payload["settled"] is True
            assert payload["status"] == "cancelled"
            await stream.read()

    agent.interrupt.assert_called_once_with("owner cancelled")
    messages = session_db.get_messages_as_conversation(session_id)
    assert messages[-1]["role"] == "assistant"
    assert "Do not continue this instruction" in messages[-1]["content"]


@pytest.mark.asyncio
async def test_session_chat_stream_persists_failure_closure(adapter, session_db):
    session_id = session_db.create_session("stream-failed-turn", "api_server")

    async def fake_run(**kwargs):
        session_db.append_message(session_id, "user", "do fragile work")
        return {
    ...
    messages = session_db.get_messages_as_conversation(session_id)
    assert [(item["role"], item["content"]) for item in messages[-2:]] == [
        ("user", "do fragile work"),
        (
            "assistant",
            "Task failed before completion: API call failed after 3 retries: HTTP 503 "
            "Do not treat this instruction as completed or continue it unless the user "
...
@pytest.mark.asyncio
async def test_confirmed_named_custom_lock_accepts_normalized_custom_runtime(adapter, monkeypatch):
    class FakeAgent:
        session_prompt_tokens = 0
        session_completion_tokens = 0
        session_total_tokens = 0
    ...
    result, _usage = await adapter._run_agent(
        ...
        conversation_history=[],
        session_id="custom-lock-session",
        route={"provider": "custom:opcnexus", "model": "private-model"},
        requested_runtime={"provider": "custom:opcnexus", "model": "private-model"},
        route_source="session_model_lock",
        confirmed_runtime_lock=True,
    )
    ...
    assert result["runtime"]["model"] == "private-model"
