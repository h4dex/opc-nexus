...
# ---------------------------------------------------------------------------
# OPC-Nexus per-conversation memory policy
# ---------------------------------------------------------------------------


class TestNexusMemoryPolicy:
    def test_rejects_partial_invalid_and_changed_policy(self):
        with pytest.raises(ValueError, match="supplied together"):
            _parse_nexus_memory_policy({"nexus_memory_mode": "none"})
        with pytest.raises(ValueError, match="scope is invalid"):
            _parse_nexus_memory_policy({
                "nexus_memory_mode": "long_term",
                "nexus_memory_scope": "../other-project",
            })
        with pytest.raises(ValueError, match="cannot change"):
            _parse_nexus_memory_policy(
                {
                    "nexus_memory_mode": "short_term",
                    "nexus_memory_scope": "employee-" + "a" * 32,
                },
                stored={"mode": "long_term", "scope": "employee-" + "a" * 32},
            )

    def test_requires_explicit_policy_for_nexus_sessions(self):
        with pytest.raises(ValueError, match="require an explicit"):
            _parse_nexus_memory_policy({}, required=True)

    def test_project_service_persists_managed_marker(self, adapter, monkeypatch, tmp_path):
        async def _exercise():
            monkeypatch.setenv("HERMES_HOME", str(tmp_path))
            monkeypatch.setenv("HERMES_NEXUS_PROJECT_ID", "project-test")
            app = _create_app(adapter)
            app.router.add_post("/api/sessions", adapter._handle_create_session)
            async with TestClient(TestServer(app)) as cli:
                missing = await cli.post("/api/sessions", json={"id": "missing-policy"})
                ...
                created = await cli.post(
                    ...
                    json={
                        "id": "managed-policy",
                        "source": "nexus",
                        "nexus_memory_mode": "none",
                        "nexus_memory_scope": "employee-" + "d" * 32,
                    },
                )
                assert created.status == 201

            session = adapter._ensure_session_db().get_session("managed-policy")
            config = adapter._parse_session_model_config(session["model_config"])
            assert session["source"] == "api_server"
            assert config["nexus_managed"] is True
            assert config["nexus_memory_policy"]["mode"] == "none"

        asyncio.run(_exercise())

    def test_create_agent_applies_scoped_file_memory_and_disables_external_provider(
        ...
        adapter._create_agent(
            ...
            memory_policy={"mode": "long_term", "scope": scope},
        )

        assert captured["skip_memory"] is True
        assert captured["memory_enabled"] is True
        assert captured["user_profile_enabled"] is True
        assert Path(captured["memory_dir"]) == tmp_path / "memories" / "employees" / scope
        assert captured["disabled_toolsets"] == []

    @pytest.mark.parametrize(
        ("mode", "expected_history"),
        [("none", []), ("short_term", [{"role": "user", "content": "earlier"}])],
    )
    def test_session_chat_enforces_history_mode(self, adapter, mode, expected_history):
        async def _exercise():
            scope = "employee-" + "b" * 32
            session = {
                "id": "s1",
                ...
                "model_config": json.dumps({
                    "nexus_memory_policy": {"mode": mode, "scope": scope},
                }),
            }
            ...
            async with TestClient(TestServer(app)) as cli:
                with (
                    patch.object(adapter, "_get_existing_session_or_404", return_value=(session, None)),
                    patch.object(adapter, "_conversation_history_for_session", new_callable=AsyncMock, return_value=history),
                    ...
                    response = await cli.post(
                        ...
                        json={
                            "message": "now",
                            "nexus_memory_mode": mode,
                            "nexus_memory_scope": scope,
                        },
                    )

            assert response.status == 200
            assert run_agent.call_args.kwargs["conversation_history"] == expected_history
            assert run_agent.call_args.kwargs["memory_policy"] == {"mode": mode, "scope": scope}
