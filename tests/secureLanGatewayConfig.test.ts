...
const {
  DEFAULT_SECURE_LAN_PORT,
  SECURE_LAN_CERTIFICATE_KEY,
  SECURE_LAN_CONFIG_KEY,
  SECURE_LAN_PRIVATE_KEY_REF,
  SecureLanGatewayController,
  SecureLanTlsIdentityStore,
  normalizeSecureLanGatewayConfig
} = await import('../src/main/services/secureLanGatewayConfig.js');
...
class MemoryStore {
  ...
  setSetting(key: string, value: unknown): void {
    if (this.failConfigWrites && key === SECURE_LAN_CONFIG_KEY) throw new Error('disk full');
...
function controller(store = new MemoryStore(), protector = new TestProtector(), gateway = new FakeGateway()) {
  const identities = new SecureLanTlsIdentityStore(store, protector);
  return {
    store,
    protector,
    gateway,
    controller: new SecureLanGatewayController(store, gateway, identities)
  };
}

const config = {
  bindHost: '127.0.0.1',
  port: DEFAULT_SECURE_LAN_PORT,
  publicHost: '127.0.0.1',
  publicPort: DEFAULT_SECURE_LAN_PORT
};

describe('SecureLanGatewayController', () => {
  it('defaults to disabled and does not auto-start without persisted intent', async () => {
    ...
    expect(harness.store.values.has(SECURE_LAN_PRIVATE_KEY_REF)).toBe(false);
  });

  it('normalizes persistent config and rejects wildcard/public listeners', () => {
    expect(normalizeSecureLanGatewayConfig({ bindHost: '192.168.1.8' })).toEqual({
      bindHost: '192.168.1.8',
      port: DEFAULT_SECURE_LAN_PORT,
      publicHost: '192.168.1.8',
      publicPort: DEFAULT_SECURE_LAN_PORT
    });
    expect(() => normalizeSecureLanGatewayConfig({ bindHost: '0.0.0.0' })).toThrow(/literal private/);
    expect(() => normalizeSecureLanGatewayConfig({ bindHost: '8.8.8.8' })).toThrow(/literal private/);
    expect(() => normalizeSecureLanGatewayConfig({ bindHost: '127.0.0.1', port: 443 })).toThrow(/port/);
    expect(() => normalizeSecureLanGatewayConfig({ bindHost: '127.0.0.1', publicHost: 'bad/host' })).toThrow(/public host/);
  });
  ...
  it('persists an encrypted private key and exposes only renderer-safe status plus pairing data', async () => {
    ...
    const stored = harness.store.values.get(SECURE_LAN_CONFIG_KEY);
    const certificate = harness.store.values.get(SECURE_LAN_CERTIFICATE_KEY) as string;
    const encryptedKey = harness.store.values.get(SECURE_LAN_PRIVATE_KEY_REF) as string;
  ...
  it('uses Electron safeStorage by default when no test protector is injected', async () => {
    ...
    const instance = new SecureLanGatewayController(store, gateway);
    await instance.start(config);
    const encrypted = store.values.get(SECURE_LAN_PRIVATE_KEY_REF) as string;
  ...
  it('preserves enabled intent on shutdown and restores the same TLS identity after restart', async () => {
    ...
    expect(store.values.get(SECURE_LAN_CONFIG_KEY)).toEqual({ enabled: true, config });
    ...
    expect(store.audits).toEqual(expect.arrayContaining([
      expect.objectContaining({ action: 'secure.lan.tls.load', result: 'ok' }),
      expect.objectContaining({ action: 'secure.lan.restore', result: 'ok' })
    ]));
  });
  ...
  it('emergency stop persists disabled before revoking sessions and prevents restart recovery', async () => {
    ...
    expect(store.values.get(SECURE_LAN_CONFIG_KEY)).toEqual({ enabled: false, config });
  ...
  it('stops a restored listener if its enabled state cannot be durably confirmed', async () => {
    ...
    expect(restored.gateway.running).toBe(false);
    expect(restored.lastError).toBe('Secure LAN Gateway restore failed');
  });

  it('certificate reset disables recovery and clears both public certificate and encrypted private key', async () => {
    const harness = controller();
    await harness.controller.start(config);
    const previousEncryptedKey = harness.store.values.get(SECURE_LAN_PRIVATE_KEY_REF);
    ...
    expect(harness.store.values.get(SECURE_LAN_CERTIFICATE_KEY)).toBe('');
    expect(harness.store.values.get(SECURE_LAN_PRIVATE_KEY_REF)).toBe('');
    expect(harness.store.values.get(SECURE_LAN_CONFIG_KEY)).toEqual({ enabled: false, config });

    await harness.controller.start(config);
    expect(harness.store.values.get(SECURE_LAN_PRIVATE_KEY_REF)).not.toBe(previousEncryptedKey);
  ...
  it('fails closed when safeStorage is unavailable or a persisted key cannot be decrypted', async () => {
    ...
    expect(unavailable.store.values.has(SECURE_LAN_PRIVATE_KEY_REF)).toBe(false);
  ...
  it('requires certificate reset before changing to an authority absent from the stored SAN', async () => {
    ...
    expect(harness.gateway.getStatus().running).toBe(false);
    expect(harness.store.values.get(SECURE_LAN_CONFIG_KEY)).toEqual({ enabled: true, config });
  });

  it('disables malformed persisted auto-start configuration instead of guessing defaults', async () => {
    const store = new MemoryStore();
    store.values.set(SECURE_LAN_CONFIG_KEY, {
      enabled: true,
    ...
    expect(store.values.get(SECURE_LAN_CONFIG_KEY)).toEqual({ enabled: false, config: null });
    expect(harness.gateway.starts).toHaveLength(0);
  });
});
