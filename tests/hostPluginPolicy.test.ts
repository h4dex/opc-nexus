...
} from '../src/main/services/hostPluginPolicy.js';
...
describe('PluginHost policy wiring', () => {
  it('maps every host permission into the narrow host capability vocabulary', () => {
  ...
  it('decides and audits every declared permission before invoking a handler', async () => {
    ...
    const host = new PluginHost(visionRegistry(), createHostPluginPermissionResolver(broker));
    const handler = vi.fn(async () => ({ ok: true }));
    host.attach('opc.nexus.vision', { 'vision.describe': handler });
    await expect(host.invoke({
      ...
      target: 'plugin:opc.nexus.vision/vision.describe'
      }
  ...
  it('audits a fail-closed decision when authenticated invocation identity is missing', async () => {
    ...
    const host = new PluginHost(visionRegistry(), createHostPluginPermissionResolver(broker));
    host.attach('opc.nexus.vision', { 'vision.describe': handler });
    await expect(host.invoke({ pluginId: 'opc.nexus.vision', capabilityId: 'vision.describe' }))
  ...
  it('admits local OCR only for the attachment read capability', async () => {
    ...
    const host = new PluginHost(visionRegistry(), createHostPluginPermissionResolver(broker));
    host.attach('opc.nexus.vision', { 'vision.ocr': handler });
    ...
    await expect(host.invoke({
      ...
      policyContext: {
        ...
        runtimeId: 'desktop-local',
        agentId: 'principal-local-admin',
        target: 'plugin:opc.nexus.vision/vision.ocr'
      }
