...
export function computeNextRun(kind: Schedule['cronKind'], value: string, from: number): number {
  ...
  const [dayPart, timePart] = kind === 'weekly' ? value.split('|') : [null, value];
...
export class Scheduler {
  ...
  create(input: ScheduleInput): Schedule {
    ...
    this.db.raw
      .prepare('INSERT INTO schedules(id, agent_id, title, cron_kind, cron_value, enabled, last_run_at, next_run_at) VALUES(?, ?, ?, ?, ?, 1, NULL, ?)')
      .run(id, input.agentId, input.title.trim(), input.cronKind, input.cronValue, next);
  ...
  private scan() {
    ...
    for (const r of due) {
      ...
      try {
        this.orchestrator.createTask(r.agent_id, r.title, 'schedule');
      } catch {
        /* 员工不存在或已停用：跳过本轮，保留计划 */
      }
    }
