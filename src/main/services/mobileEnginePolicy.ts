...
export function androidOperatorEngineError(kind: AgentKind, engineId: string): string | null {
  ...
  return `Android 手机操作员仅支持 Hermes Agent CLI（${ANDROID_OPERATOR_ENGINE_ID}）；当前配置为 ${engineId}。其他执行引擎目前没有 Android 工具桥接。`;
}
...
export function androidOperatorRuntimeUnavailableError(): string {
  return 'Android 手机操作员所需的 Hermes Agent CLI 当前不可用。手机任务不会降级到其他执行引擎，因为这些 Runtime 目前没有 Android 工具桥接。';
}
