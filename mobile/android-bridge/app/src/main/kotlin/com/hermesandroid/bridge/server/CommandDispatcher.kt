...
/**
 * Single source of truth for commands received over the authenticated outbound WSS
 * connection managed by RelayClient. The Android app never opens a listening socket.
 *
 * @param authenticated whether the caller is authenticated. The relay connection is
 *   authenticated through pairing plus challenge signing before dispatch. Only `/ping` reports it.
 */
    ...
    suspend fun dispatch(
        ...
        return when {
            ...
            method == "GET" && path == "/screen" -> {
                ...
                mapOf(
                    "tree" to tree,
                    "count" to countAllNodes(tree),
                    "hash" to tree.joinToString("|") { it.computeHash() }
                ) to 200
            }
